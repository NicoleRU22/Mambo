...
const { validateProduct, validateId, validateProductQuery } = require('../middleware/validation');
...
router.put('/:id', authenticateToken, requireAdmin, validateId, validateProduct, async (req, res) => {
  try {
    const { id } = req.params;
    const {
      name,
