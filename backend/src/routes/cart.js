...
router.post('/checkout', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id;
    const { shipping_address, shipping_city, shipping_state, shipping_zip_code, shipping_phone, payment_method, notes } = req.body;
    ...
    const [orderResult] = await query(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [userId, orderNumber, total, shipping_address, shipping_city, shipping_state, 
        shipping_zip_code, shipping_phone, payment_method, notes || null]);
    ...
    for (const item of cartItems) {
      await query(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `, [orderResult.insertId, item.product_id, item.name, item.price, item.quantity, item.size]);
